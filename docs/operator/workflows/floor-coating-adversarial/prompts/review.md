# Floor-Coating Adversarial Review

You are reviewing D010, the current flooring proposal:

> Durabak-18 Outdoor Textured light grey, 3 gal, applied to floor+cove+ramp; fuel/oil spill-resistant, clean promptly; PlexCore adhesion and fuel-drip patches before full coat.

Read:

- `docs/juplaya-trailer-context.md`
- `docs/DECISION_LOG.md`
- `docs/research/build-decision-validation.md`
- `docs/research/floor-coating-evidence-2026-06-05.md`

The owner likes Durabak for durability and low weight, but cost is substantial. Your assigned posture is in the job objective. Argue from that posture and try to break the current recommendation.

Score each viable candidate on:

- total material cost for floor+cove+ramp
- weight and payload impact
- gasoline/diesel/oil spill tolerance
- playa dust and hose-out cleanup
- ability to make the 3–4" wall cove before FRP lands
- DIY execution risk on PlexCore
- repairability after gouges from bikes, chocks, and E-track fittings
- deadline risk before Juplaya

Output:

1. Recommended floor system.
2. Why your posture accepts or rejects Durabak.
3. Strongest counterargument to your recommendation.
4. Required patch tests and install notes.
5. One-line verdict for D010: keep, revise, or replace.
