...
Why these spots survive every other system: the 12" cutout fits **inside one 16"-OC post bay (~14.5" clear) — no post gets cut, no sub-frame needed.** The 36–58" window band sits between the seat/bed track (now ~16") and the shelf track (60") — well above the ~20" sofa/bed surface for a good seated sightline. Roadside-rear is behind the bikes' handlebar sweep (bars live in the front half, nose-forward). Curbside-rear clears the 18.6"-tall fridge and the roofline-mounted awning case above.
...
- **Seat/bed row at ~16", shelf row at ~60"**, both lag-backed into the 16"-OC tube posts.
- **Height — dual duty (revised 2026-06-05: daytime sofa/bench AND bed).** The platform is both the seat and the bed. **Target ~20" surface = ~16" seat/bed row + deck + ~5" dual-purpose foam** (mattress by night, seat cushion by day). 20" was chosen over the seat-optimal ~18" to keep **more under-deck storage (~12–14" clear) and an easier bed to rise from**; it still seats fine (a touch high for shorter users, but the ~5" cushion compresses toward ~19"). It also far outdoes the old ~31" surface on sit-up-in-bed headroom (~56" above vs ~46"), so it serves both jobs. The old "34"→27" for headroom" reasoning is superseded. Backrest = cushions against the wall (~20"→34"), under the 36" window sill. Bikes are out in camp mode, so nothing competes for the height.
- **Gate (dual sit-test, design-freeze item 3): mock a ~20" surface and pass BOTH (a) sit on the edge, feet flat — seat comfort, and (b) sit up in bed — headroom, before drilling.**
...
| 3 | **Track heights final** — seat/bed ~16" (~20" surface, dual duty), shelf 60" | **dual** mock at ~20": sit-on-edge feet-flat (seat) + sit-up-in-bed (headroom) both pass; shelf checked against the 36–58" window band | ☐ |
