...
- **Selected fixture class:** seven **Super Bright LEDs `VAL2-NW9`** floods (9", black, 1450 lm, 18 W, 4000 K, 90 deg, IP67, 12-28 VDC) are the current recommendation from the [exterior-lighting panel](../runs/exterior-lighting-panel/synth/VERDICT.md). `VAL2-WW9` is the warm 3000 K alternate if campsite ambience matters more than task clarity. Use separate warm 24 V dimmable strip lighting for the awning, such as **`RA-IP68-80CRI-5m` 3000 K**, not another glare flood.
...
- **Accessories:** Blue Sea 5026 · Scanstrut SC-USB-F3 · LandAirSea 54 · locks (above) · 14 AWG runs + fuse assortment · fused cigarette-lighter receptacles for the power cabinet · dome/task lights (24 V preferred; 12 V only at the cabinet outlets) · **exterior lighting: 7 × Super Bright LEDs `VAL2-NW9` floods (`VAL2-WW9` warm alternate), 1 × warm 24 V awning strip such as `RA-IP68-80CRI-5m`, optional amber step/courtesy, plus cabinet switches/dimmer**.
