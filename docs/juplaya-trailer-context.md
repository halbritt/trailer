...
10. **Nose cabinet** — battery, SmartSolar, shunt, breaker, Orion-Tr 48/24, Orion-Tr 48/12, 5026, fused 12 V receptacles; rail wiring out to branches; cabinet venting with low filtered intake, high RecPro RP-2414 exhaust only if it avoids forward-facing tow rain, 24 V fan, and temperature switch. If the only exterior location is high on the nose, add a closable travel shutter/damper or vent cabinet-to-cabin instead. MultiPlus mounting space can be reserved for Phase 2, but it is not a Juplaya install dependency.
