...
- **Exterior lighting layout:** **2 curbside floods, 2 roadside floods, 1 flood on each V-nose face, 1 rear flood, plus separate awning lighting**. Optional low amber step/courtesy can share the awning circuit if added. Run them from the **24 V house bus** using 12-28 VDC / 10-30 VDC IP67-ish fixtures, fused per branch, and switched at the cabinet. Keep all house exterior lighting isolated from the OEM trailer lights and 7-way tow plug.
...
- **Switch hardware:** use **Blue Sea 8260 6-position Contura mounting panel + 6x Blue Sea 8282 SPST OFF-ON black Contura switches** labeled INTERIOR, CURB FLOOD, ROAD FLOOD, NOSE FLOOD, REAR FLOOD, AWNING. The 8260 panel needs <=0.38" mounting thickness, so use a thin inset switch plate or rabbet a thicker cabinet face. For the awning strip, add a **Super Bright LEDs `LDK-8A` 12-24 V / 8 A PWM dimmer** after the AWNING switch; Blue Sea 7509 is the premium marine dimmer fallback.
...
- **Proven 2178 coupler lock** for the documented 2" coupler + **2× PACLOCK 7-Series / UCS-7A flat-back puck locks, keyed alike** + **Trimax TCL65** wheel lock — layered, because Juplaya. One keyed-alike puck goes on the rear ramp; one goes on the personnel-door hasp. Do not order the Proven 2516 unless the trailer is upgraded to a 2-5/16" coupler.
...
- **Ramp door:** first choice is the matching keyed-alike PACLOCK 7-Series / UCS-7A puck if the ramp latch/hasp accepts a puck. If the ramp only has a conventional shackle hole, either add a puck-compatible hasp or use a PACLOCK keyed-alike UCS shackle padlock as the fallback.
- **LandAirSea Overdrive GPS + hardwire cable preferred**, hardwired to the 24 V block (3 A branch), always-on. The normal LandAirSea 54 + USB-C hardwire is acceptable if the tracker must ship immediately, but the Overdrive's larger backup battery and stronger antennas are a better fit for Oakland storage. This remains the primary hidden keepalive/tracking layer; Starlink Mini is a secondary high-bandwidth path only if the storage spot has open sky and the power schedule works. A free/covered Oakland spot is still usable with cellular telemetry; choose an Alameda/open-sky spot only if physical security, access, solar exposure, or broadband telemetry are worth the delta.
