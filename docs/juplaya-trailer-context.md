...
**Why this AIO:** the original LiTime 5 kW unit had a 120–450 V PV window — and no panel string that physically fits this roof can reach 120 V cold-safe (the math is in the Solar section). It went back. The **3500 W** unit's 60–145 V window fits the strings the roof can actually carry. That story is the clearest example of why gates exist: spec-sheet architecture died on a roof measurement.
...
- **NEVER 3S.** Three panels in series hit **157–163 V cold Voc — over the AIO's 145 V absolute max.** A third roof panel would need its own MPPT; the roof can't fit one anyway.
